...
superpose_all_languages(list, limit):
  for i=0, i++, i < len(list):
    for j=i+1, j++, j < len(list)
      sp = superpose_lanugages(list[i], list[j], limit)
      if sp = {}:
        # inconsistent
...
\end{verbatim}
\end{figure}
\captionof{figure}{Pseudo-code for superposing a list of languages of strings.}\label{other:pseudo-code-spsens}
\end{center}
...
\begin{center}
	\begin{figure}[h!]
		...
		\end{itemize}
	\end{figure}
	\captionof{figure}{TLINKs from the wsj\_1073 document of TimeBank 1.2.}
\end{center}
...
% Proof of concept tool rather than comparison with other annotation tools,
% illustrates what has been enabled by strings

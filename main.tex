...
\begin{center}
	...
	\end{tabular}
\end{center}
This gives that the event time of `left' $E_{leave}$ occurs before the event time of `bought' $E_{buy}$.
	
However, in order to treat the Progressive aspect which English may use, the so-called `point of the event' must in fact be taken as an interval rather than an instantaneous point, indicating the durative nature of the Progressive. \citet{Derczynski2013} treat this by splitting $E$ into $E_s$ and $E_f$ for the beginning and ending of $E$, respectively.
%In fact, all three time points (typically labelled as $E$ for event time, $R$ for reference time, and $S$ for speech time) may be considered as intervals\footnote{Using combinations of the Progressive and Perfective aspects is admittedly more difficult. For instance, the only difference in \citet{reichenbach1947elements} between Past Perfect and Past Perfect Progressive is that $E$ is treated as an interval in the latter. } while maintaining the initial arrangements to represent the tenses and aspects. This is presented using the Allen relations (see \cref{ssub:allen}) in \Cref{fig:reichenback-arrangements} below.

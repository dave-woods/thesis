\documentclass[a4paper,12pt,leqno,twoside]{article}
...
\date{\parbox{\linewidth}{\centering%
		...
		{\Large \textbf{University of Dublin, Trinity College}}\\\endgraf April 2022%\monthyeardate\today
		}{\small \vspace{\fill} Supervised by: Dr Tim Fernando, Dr Carl Vogel}}

\makeindex[title=Index of Definitions and Concepts, intoc]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                              %
%                                Preamble Begins                               %
%                                                                              %
...
\end{enumerate}\index{Allen Relations!Motivations}
...
\end{center}\index{Allen Relations!Symbols}
...
The framework proposed by Allen uses a directed graph as its basis, in which the nodes represent intervals, and the arcs are labelled according to the relation (or, in the case of uncertainty, relations---see \Cref{tab:allen-trans-table}) between the intervals. It is assumed that complete information about the relations in the network is maintained---that is, there are no nodes without an edge between them, as the transitivities\index{Allen Relations!Transitivity} between the various relations are to be computed as necessary---for instance, on the addition of a new interval into the network.
...
\begin{center}
	...
	\end{figure}
\end{center}
For $N$ nodes (intervals) there are $\frac{N^2-N}{2}$ edges if all new nodes are connected to all existant nodes when they are created. While in \Cref{fig:simple-transitivity} there is one label per edge, there may be as many as $13 \cdot \frac{N^2-N}{2}$ labels per edge if there is no knowledge about any relations. In \Cref{fig:complex-transitivity}, it can be seen that a pair of single-label edges may result in a multiple-label edge by transitivity: the relations $X$ o $Y$ and $Y$ o $Z$ results in the disjunction of $X$ \textless $Z$, $X$ o $Z$, and $X$ m $Z$. Without further data, there is no way to definitively constrain this disjunction to a single relation. If there is a single label for every edge in the graph, this is known as \textit{temporal closure}\index{Temporal closure}---as discourse often naturally features incomplete or vague temporal information, calculating the temporal closure is, in general, a difficult goal to achieve, though it is desirable in order to create a representation that is both complete and consistent \citep{Verhagen2005}.
\begin{center}
	\begin{figure}[h!]
...
A fluent\index{Fluent} is a condition which may change over time---thus a predicate such as $\allowbreak{}Sleeps(john)$ (``John sleeps'') becomes the fluent $Sleeps(john, t)$, where $t$ is the time at which John is sleeping. Following the convention set out and used by \citet{Mccarthy69somephilosophical}, \citet{van2008proper}, \citet{fernando2016prior} (among others), a fluent may be understood here as naming a temporal proposition---some event, time period, or state which may change (which hereafter will also be referred to as an event, as in \citet{Pustejovsky2005}). Sets of fluents will be encoded as symbols so that any number of them may hold at once, and these symbols will make up the alphabet from which strings are created.
...
\end{align}\index{Conflation of language and string}
...
\end{alignat}\index{MSO!String positions}
...
\begin{align}
	[n] := \{1,2,\ldots,n\}
\end{align}
Since $s$ is restricted to being a finitely bounded string, $[n]$ is also finite, and thus the MSO model described by $s$ must also be finite.\index{MSO!Finite models}
...
\end{align}\index{MSO!Models described by strings}
...
\end{align}\index{Analogy!Strings}
...
\end{align}\index{Analogy!Languages}
...
\end{align}\index{Pointwise strings!$l$ and $r$}
...
\begin{align}
	...
	\end{cases}
\end{align}\index{Pointwise strings!Translation from intervals}
...
\end{cases}
\end{align}\index{Block compression!1@\bc}
...
\end{align}\index{Block compression!2@$\bc^{-1}$}
For example:
...
\end{align}
Since these strings all block compress to the same string, they can be said to be equivalent under block compression. Specifically, strings $s$ and $s'$ are \textit{\bc -equivalent}\index{Block compression!3@\bc -equivalent} iff $\bc(s) = \bc(s')$. This ability to generate infinitely many strings which have an equivalent interpretation allows for varying the length of a string as will be required in order to form a useful notion of superposition (see p. \pageref{def:initial-async-superposition}).
...
\end{align}\index{Superposition!Basic $\sp$}
For example:
...
\end{align}\index{Superposition!Basic $\sp$!Languages}
...
\end{align}\index{Block compression!4@$pad_k$}
...
\end{align}\index{Projection!Strings}
...
\end{align}\index{Projection!Languages}
...
\end{align}\index{Superposition!Vocabulary-constrained $\spvc$}
...
\end{align}\index{Allen Relations!String representations}
...
Now, superposing all of the languages in \Cref{tab:zebra-clue-strings} and taking into account the constraints in \cref{impl:zebra-constraintsA,impl:zebra-constraintsB,impl:zebra-constraintsC}, a single result string is generated (each Value is displayed here abbreviated to its first two letters):
\begin{align}
...
\end{align}\index{Semi-intervals!$\alpha$ and $\omega$}
...
\end{align}\index{Semi-intervals!$semi(s)$}
...
\end{align}\index{Superposition!Sensible $\sensp$}
...
\end{align}\index{Reasoning!Contains}\index{Reasoning!Contradicts}
...
%, showing strings in use as part of a semantic test suite.%, and ensuring that the code implementing the framework is valid and behaves as expected.
...
%; and demonstrating the use of strings to represent the inferences which appear within the FRACAS semantic test suite.%; and verifying that the implementation code produces exactly and only the expected results.
% \subsection{Timeline Validity}\label{sub:validity}
% The string framework allows for compact temporal data representation with an intuitive link to timelines due to the sequential nature of the data, as explored in \cref{ssub:timelines}. However, it is crucial that these string representations do not mislead users by creating contradictory or otherwise problematic results, or if the information being operated on becomes mutated in some way that would lead to incorrect inferences being drawn that would not have been drawn before the data was manipulated. In essence, it must be ensured that when strings containing data from a source such as a document that is marked up with TimeML from the TimeBank corpus are utilised in any way, but particular after superposition as the most heavily relied upon operation in the framework, that no information becomes lost or falsified in the process.
